...
`Address` et `Letting` à des fins de test. Ces factories utilisent `factory_boy`
pour créer des objets avec des données réalistes, simplifiant ainsi les tests.
...
class AddressFactory(factory.django.DjangoModelFactory):
    ...
    Génère des instances du modèle `Address` avec des données réalistes pour chaque
    champ, y compris le numéro de bâtiment, la rue, la ville, l'état, le code postal,
    et le code pays (ISO).
...
class LettingFactory(factory.django.DjangoModelFactory):
    ...
    Génère des instances du modèle `Letting`, incluant un titre aléatoire et une
    adresse associée générée par `AddressFactory`.
